...
%E13
p   = 10e6;         %[Pa]    
Di  = 70e-3;        %[m] insida cylinder(anv?nd till l?ngd gavel)
Dy  = 76e-3;        %[m] 
d   = 30e-3;        %[m]
l   = 250e-3;       %[m] l?ngd cylinderr?r
h   = 35e-3;        %[m] l?ngd distansring
b   = 8e-3;         %[m]
...
m     = Skruv(4)*10^-3;    %metriskg?nga
dh    = Skruv(5);          %frig?ende h?l, medel
P     = Skruv(6);          %stigning
...
%% Styvheter
%Alla ?r seriekopplade
cs_ut=4*((E_steel*Area_skruv)/Lskruv);      %hela f?rbandet, varje skruv bidrar med styvhet 
...
%Glapp n?r Fk=0, F0 minsta till?tna v?rde, F0 m?ste vara st?rre ?n detta
...
F       = [Fs_ut Fk_in]; 
Asp     =  4*((pi/16)*(d1+d2)^2); 
sigma_a = (max(F)-min(F))/(2*Asp);  %hela f?rbandet

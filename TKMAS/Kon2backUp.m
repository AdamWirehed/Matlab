...
%% Variables
p=10e6; %Multiplied by 1.25 to calculate the stress with an 25% increase
...
F0_in=Ck_in/(Cs_in+Ck_in)*(Fn_in*1.25); %F?rsp?nning f?r att f?rhindra glapp
F0=[F0_in F0_ut];

delta_ut=max(F0)*(1/Cs_ut+1/Ck_ut);
Cs_utpy=[0 Cs_ut*delta_ut];
Cs_utpx=[0 delta_ut];
Ck_utpy=[0 Ck_ut*delta_ut];
Ck_utpx=[delta_ut 0];

delta_in=max(F0)*(1/Cs_in+1/Ck_in);
...
Fk_in=max(F0)-Ck_in/(Cs_in+Ck_in)*(Fn_in); 
Fs_ut=max(F0)+Cs_ut/(Cs_ut+Ck_ut)*(Fn_ut);
Fs_in=max(F0)+Cs_in/(Cs_in+Ck_in)*(Fn_ut);
...
Mtot_min=max(F0_es)*(0.16*P+0.58*my_min*d2+myb_min*rm);

F0_wc1=Mtot_max/(0.16*P+0.58*my_min*d2+myb_min*rm);
F0_wc2=Mtot_min/(0.16*P+0.58*my_max*d2+myb_max*rm);

F0_wc=[F0_wc1 F0_wc2];
%% Max Stress
Fs_utes=max(F0_wc)+Cs_ut/(Cs_ut+Ck_ut)*(Fn_ut*1.25);
...
%% Important Variables
fprintf('Mtot:%2.2d \nF0:%2.2d \nF0_es:%2.2d \nsigma_a:%2.2d \nsigma_max:%2.2d \n',Mtot_max,max(F0),max(F0_es),sigma_a,sigma_max);
